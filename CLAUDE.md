...
- **거래정지일은 관측 구간에서 배제한다** — 이 DB에서는 **`v=0` · `O=H=L=C`(가격 동결)**로 남고
  **전 일봉의 2.36%**(45,818행, 2026-09-05 실측)다. `O/H/L=0` 모양은 상위 스키마의 `check (o > 0 …)`가
  거부해 **아예 들어오지 않는다.** 구간의 **양 끝이 거래일**이어야 하고(끝이 정지면 종가가 어제 것이다),
  달력은 **지수에서** 센다 — 종목 달력으로 세면 「5거래일 뒤」가 종목마다 다른 날이 된다.
...
**M0~M6 완료 · 84/108 (78%)** (2026-09-05) — SPEC **v0.7** · PLAN **v0.3**.
결정은 V1~V13 확정, **V14(MCP 서버를 어디서 만드나)만 의도적으로 미뤄 두었다.**
진도는 `docs/TASKS.md` 대시보드 참조.

**돌아가고 있는 것** — 상위 `alert.yml`이 평일 08:20 KST에 이 리포를 깨우고(`repository_dispatch`),
예비 cron이 09:05 KST에 `--if-not-verified`로 한 번 더 묻는다. 판정은 `ksv_verdicts`에,
5·20·60거래일 채점은 `ksv_outcomes`에, 분포는 `ksv_discrimination`에 쌓인다. 메일은 `[검증]`으로 간다.

**남은 것** — M7 웹 4화면(**DESIGN 시안 합의가 선행 조건**) · M8 상위 공매도 수집(사용자 승인) · M9 마무리.
**M-1 ②c 신용잔고**는 미해결이고 이 프로젝트 배점 밖이다.
