...
| ①b ✅ 2026-09-07 | `krx-stock-charts` | 일일 갱신에 **공매도 수집 2회**를 더하고 `ksc_shorting`에 적재 (상위 F15 · `7d9fe04`) | V6b. `get_shorting_volume_by_ticker`가 시장당 1회로 전 종목을 준다(KOSPI 944행 실측). 수급(선행 D22)과 같은 패턴이라 상위 코드에 이미 자리가 있다. **없어도 되는 층이다** |
